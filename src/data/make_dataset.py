...
def train_val_split(data:pd.DataFrame , 
    ...
    dataset_logger.save_logs(msg = f'Data is split into train split with shape {train_data.shape} and val split with shape {val_data.shape}',
                             log_level='info')
    dataset_logger.save_logs(msg = f'The paremeter values are {test_size} for test_size and {random_state} for random_state' ,
                             log_level= 'info')
...
def main():
    ...
    # make directory for interim data 
    interim_data_path.mkdir(exist_ok=True , parents= True)
    # raw train file path
    raw_df_path = root_path/'data'/'raw'/'extracted'/'train.csv'
    ...
    save_data(data = val_df , output_path= interim_data_path/'test.csv')
