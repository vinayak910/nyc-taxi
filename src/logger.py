...
def create_log_path(module_name:str)-> str:
    """
    Create a log file path based on current date and the provided module name
    ...
    #create a folder for specific module
    module_path_log = log_dir_path/ module_name
    module_path_log.mkdir(exist_ok=True , parents = True)

    # convert the current date into string
    current_date_str = current_date.strftime("%d-%m-%Y")
    # create log files based on current date
    log_file_name = module_path_log/(current_date_str + '.log')
...
class CustomLogger:
    ...
    def save_logs(self , msg , log_level = 'info'):
        """
        Saves logs to the specific log file with the given message and log level.
        ...
        logger = self.get_logger()
        # save the logs to the file using given message
        if log_level == 'debug':
